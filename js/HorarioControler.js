...
function MostrarDiasSeleccionados() {
    ...
    let hayHorarioPropio = false;
    let hayHorarioPorDefecto = false;

    for (let i = 0; i < idsDiasSeleccionados.length; i++) {
        let esSubsistema = false;
        ...
        let dia = semana.Horario.find(d => d.IdDiaSemana === idDia);
        let tipoHorarioSemanal = String.fromCharCode(semana.TipoHorarioSemanal);
        let tipoHorarioDiario = String.fromCharCode(dia.TipoDeHorarioDiario);


        if ((tipoHorarioSemanal == "L" && (tipoHorarioDiario == "C" || tipoHorarioDiario == "S")) ||
            (tipoHorarioDiario == "S" && (tipoHorarioSemanal == "L" || tipoHorarioSemanal == "C")) ||
            (tipoHorarioSemanal === "G" && tipoHorarioDiario === "S")) {
            esPorDefecto = true;
            if (!hayHorarioPropio) {
                let checkbox = document.getElementById("ChkHorarioSubsistema");
                checkbox.checked = true;
                HorarioSubsistema();
                hayHorarioPorDefecto = true;
            }
            //// FInalmente NO
            //PasarHorarioAParticularYSeleccionado(idDia);
            //GeneraListadoHorarioParticular();
        }
        else if (dia.SinServicio) {
            esSubsistema = true;
            if (!hayHorarioPropio && !hayHorarioPorDefecto) {
                let checkbox = document.getElementById("ChkSinServicio");
                checkbox.checked = true;
                SinServicio();
            }
        }
        else if (!esSubsistema && !esPorDefecto) {
            PasarHorarioAParticularYSeleccionado(idDia);
...
function AplicarHorario() {
    ...
    if (!chkSinServicio.checked && (chkHorarioSubsistema === null || !chkHorarioSubsistema.checked)) {
        if (lis.length === 0) {
            AddHorario();
        }
    ...
    else if (chkHorarioSubsistema !== null && chkHorarioSubsistema.checked) {
        // relleno todo a cero
        RellenaHorarioSeleccionado(0, 48, 0);
        horario = []; // Lo envío vacío para indicar que no hay actividad. Si es de algún grupo tomará el del subsistema
    }
    ...
    //Parámetros
    let params = { "diasSeleccionados": diasSeleccionados, "horarioSeleccionado": horario, "semanal": semana };

    $.ajax({
        type: "POST",
        url: url + "/Horario/ActualizarHorario",
        ...
        success: function (resultado) {
            ...
            for (let i = 0; i < diasSeleccionados.length; i++) {
                ...
                let horarioDia = semana.Horario.find(d => d.IdDiaSemana == idDia);
                if (horarioDia != null) {
...
function GuardarDatos(salir) {
    ...
    $.ajax({
        type: "POST",
        url: url + "/Horario/GuardaHorario",
        data: JSON.stringify(semana),
        contentType: "application/json; charset=utf-8",
        async: false,
        cache: false,
        timeout: 5000,
        success: function (resultado) {
            ...
            if (respuesta.error === "S") {
                ...
                for (let i = 0; i < 7; i++) {
                    let dia = semana.Horario.find(d => d.IdDiaSemana === i);
...
/// --------------------------------------------------------------
/// Activa que aparezca el combo para seleccionar los mensajes
/// o para ocultar según el estado del checkbox
/// --------------------------------------------------------------
function ActivaDesactivaMensajeFueraHorario() {
    let checkMensaje = document.getElementById("labelcheck");

    if (checkMensaje !== null && checkMensaje !== 'undefined') {
        if (checkMensaje.checked) {
            $("#labelMensajeFueraHorario").text("Mensaje fuera de horario");
            $("#SelectMensaje").css("display", "block");
            $("#textoWellMensaje").css("display", "block");
        }
        else {
            $("#labelMensajeFueraHorario").text("Asignar mensaje de fuera de horario");
            $("#SelectMensaje").css("display", "none");
            $("#textoWellMensaje").css("display", "none");
        }
    }
}
...
/// -----------------------------------------------------------------------
/// Cuando cambia el mensaje fuera de hora se debe cambir el texto que se
/// muestra al lado del combo y actualizar el seleccionado en la variable
/// semana
/// ------------------------------------------------------------------------
function CambiarTextoMensaje() {
    // obtengo el id del mensaje
    let idMensaje = $("#SelectMensaje option:selected").val();

    if (idMensaje === "0") {
        $("#TextoWellMensaje").html("");
        $("#TextoWellMensaje").css("display", "none");
        semana.IdMensaje = "0";
        semana.Mensaje = "";
    }
        ...
        let textoMensaje = ""
        // lo busco en el array de mensajes
        let mensaje = mensajes.find(m => m.Description == idMensaje);
        // Debería existir, pero por si acaso
        if (mensaje) {
            textoMensaje = mensaje.TextoMensaje;
            // Cambio el identificador del mensaje de fuera de horrio de la semana
            semana.IdMensaje = idMensaje;
            semana.Mensaje = mensaje.TextoMensaje;
        }

        $("#TextoWellMensaje").html(textoMensaje);
        $("#TextoWellMensaje").css("display", "block");
    }
}
