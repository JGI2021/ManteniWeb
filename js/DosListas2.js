    ...
    MultiSelect.prototype = {
        ...
        'generateLisFromOption': function (option, index, $container) {
            ...
            selectableLi
              ...
              .attr('id',/*idCont + "-" + */elementId + '-selectable');

            selectedLi
              .data('ms-value', value)
              .addClass('ms-elem-selection')
              .attr('id', /*idCont + "-" + */elementId + '-selection')
              .hide();
            ...
            if ($optgroup.length > 0) {
                ...
                if ($selectableOptgroup.length === 0) {
                    ...
                    if (that.options.selectableOptgroup) {
                        $selectableOptgroup.find('.ms-optgroup-label').on('click', function (evt) {
                            if (evt.ctrlKey)
                                alert("ctrl+key");
        ...
        'deselect': function (value) {
            ...
            var that = this,
              ...
              msIds = $.map(value, function (val) { return (that.sanitize(val)); }),
              selectables = this.$selectableUl.find('#' + /*idCont + "-" +*/ msIds.join('-selectable, #') + '-selectable'),
              selections = this.$selectionUl.find('#' + /*idCont + "-" +*/ msIds.join('-selection, #') + '-selection').filter('.ms-selected').filter(':not(.' + that.options.disabledClass + ')'),
